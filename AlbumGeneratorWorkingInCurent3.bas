Attribute VB_Name = "AlbumGeneratorWorkingInCurent3"
' Module: CorelDRAW BuildAlbum - Enhanced
' Enhanced version: preserves legacy behavior optionally and adds flexible folder-structure parsing
Option Explicit
...
' Опция: если True — используем старую логику формирования подписей (GetCaptionsForFolder_Legacy)
' если False — используем новую, более гибкую логику (GetCaptionsForFolder_Enhanced)
Public g_UseLegacyLogic As Boolean
...
Private Function NormalizeName(ByVal s As String) As String
    Dim t As String
    t = LCase$(s)
    ' Заменим разделители на пробелы, уберём лишние символы
    Dim i As Long, ch As String
    For i = 1 To Len(t)
        ch = Mid$(t, i, 1)
        If (ch >= "a" And ch <= "z") Or (ch >= "0" And ch <= "9") Or (ch >= "а" And ch <= "я") Or ch = " " Then
            NormalizeName = NormalizeName & ch
        Else
            NormalizeName = NormalizeName & " "
        End If
    Next i
    ' Сжать пробелы
    NormalizeName = Trim$(Replace(NormalizeName, "  ", " "))
End Function
...
' ================== Сбор целевых папок (один или два уровня) ==================
Function CollectTargetFolders(ByVal rootPath As String, ByVal depth As Integer) As Collection
    ' Сохранена для обратной совместимости — возвращает Collection объектов Folder (как раньше)
    Dim fso As Object, rootFld As Object, subFld As Object, subSub As Object
    Set fso = CreateObject("Scripting.FileSystemObject")
    ...
    For Each subFld In rootFld.SubFolders
        If depth = 1 Then
            col.Add subFld
        ElseIf depth >= 2 Then
            Dim hasSub As Boolean: hasSub = False
            For Each subSub In subFld.SubFolders
                hasSub = True
                col.Add subSub
            Next subSub
            If Not hasSub Then col.Add subFld
        End If
    Next subFld

    Set CollectTargetFolders = col
End Function
...
Sub BuildAlbum_Enhanced()
    ...
    ' Выбираем режим: legacy или enhanced
    Dim mode As String
    mode = InputBox("Режим обработки (1 - legacy, 2 - enhanced). Введите 1 или 2:", "Режим обработки", "2")
    If Trim(mode) = "1" Then g_UseLegacyLogic = True Else g_UseLegacyLogic = False

    ' Глубина поиска папок: 1 (по подпапкам корня) или 2 (категория -> подпапки)
    Dim depthStr As String
    depthStr = InputBox("Уровень сканирования подпапок (1 или 2):", "Глубина сканирования", "2")
    Dim depth As Integer
    depth = val(depthStr)
    If depth < 1 Then depth = 1

    Dim folders As Collection
    Dim useKvStructure As Boolean: useKvStructure = False
    Dim tmpFld As Object
    Set folders = New Collection
    ' Автоопределение: если в корневой папке есть каталоги с "кв" — используем специальную логику
    Dim fso As Object: Set fso = CreateObject("Scripting.FileSystemObject")
    Dim rootFld As Object: Set rootFld = fso.GetFolder(rootPath)
    For Each tmpFld In rootFld.SubFolders
        If InStr(1, LCase$(tmpFld.Name), "кв", vbTextCompare) > 0 Then
            useKvStructure = True: Exit For
        End If
    Next tmpFld

    If useKvStructure Then
        Set folders = CollectFoldersForKvStructure(rootPath)
    Else
        Set folders = CollectTargetFolders(rootPath, depth)
    End If
    ...
    ' Вставляем стартовые подписи (как раньше)
    ...
    For Each fldInfo In folders
        ...
        If IsObject(fldInfo) Then
            ...
            kvNum = ExtractDigits(folderDisplayName)    ' если есть номер квартала в имени — возьмём
