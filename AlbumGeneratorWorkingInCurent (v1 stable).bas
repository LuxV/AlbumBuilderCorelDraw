...
' Пытаемся создать document-level guides; если невозможно — fallback = page guides
Sub AddGuidesToDocument(ByVal doc As Document)
    Dim OK As Boolean: OK = False
    On Error Resume Next
    Dim testLayer As Object
    Set testLayer = doc.GuidesLayer
    If Err.Number = 0 And Not testLayer Is Nothing Then
        ' создаём направляющие в документном слое
        Dim wInch As Double, hInch As Double
        doc.ActivePage.GetSize wInch, hInch
        testLayer.CreateGuide MMtoDocUnits(V_GUIDE_LEFT), 0, MMtoDocUnits(V_GUIDE_LEFT), hInch
        testLayer.CreateGuide MMtoDocUnits(V_GUIDE_RIGHT), 0, MMtoDocUnits(V_GUIDE_RIGHT), hInch
        testLayer.CreateGuide 0, MMtoDocUnits(H_GUIDE_TOP), wInch, MMtoDocUnits(H_GUIDE_TOP)
        testLayer.CreateGuide 0, MMtoDocUnits(H_GUIDE_BOTTOM), wInch, MMtoDocUnits(H_GUIDE_BOTTOM)
        OK = True
    End If
    Err.Clear
    On Error GoTo 0
    ...
    If OK Then
        g_DocGuidesSupported = True
    Else
        g_DocGuidesSupported = False
        ' fallback: для каждой существующей страницы создаём page-guides
        'Dim p As Page
...
' ========== Natural sort (без расширения, регистронезависимо) ==========
...
' Возвращает следующий токен (числовой или текстовый) из строки s, начиная с позиции pos.
' После вызова pos указывает на первый символ после возвращённого токена.
Private Sub NextToken(s As String, ByRef pos As Long, ByRef token As String, ByRef isNum As Boolean)
    token = ""
    isNum = False
    Dim L As Long: L = Len(s)
    If pos > L Then Exit Sub
    Dim ch As String
    ch = Mid$(s, pos, 1)
    If IsDigit(ch) Then
        isNum = True
...
' Возвращает -1, 0 или 1 в зависимости от сравнения a и b по natural-правилам.
' Ожидается, что входные строки — уже без расширения и в нужном регистре (но внутри используется vbTextCompare).
Private Function NaturalCompare(a As String, b As String) As Long
    Dim posA As Long: posA = 1
    Dim posB As Long: posB = 1
    Dim lenA As Long: lenA = Len(a)
    Dim lenB As Long: lenB = Len(b)
    Dim tokA As String, tokB As String
    Dim isNumA As Boolean, isNumB As Boolean
    
    Do While posA <= lenA Or posB <= lenB
        Call NextToken(a, posA, tokA, isNumA)
        Call NextToken(b, posB, tokB, isNumB)
        
        If tokA = "" And tokB = "" Then Exit Do
        If tokA = "" Then
        ...
        If isNumA And isNumB Then
            ...
            numA = val(tokA): numB = val(tokB)
            If numA < numB Then NaturalCompare = -1: Exit Function
            If numA > numB Then NaturalCompare = 1: Exit Function
            ' если числовые значения равны — короче токен (меньше ведущих нулей) первым
            If Len(tokA) < Len(tokB) Then NaturalCompare = -1: Exit Function
            If Len(tokA) > Len(tokB) Then NaturalCompare = 1: Exit Function
        ElseIf isNumA And Not isNumB Then
            NaturalCompare = -1: Exit Function
        ElseIf Not isNumA And isNumB Then
            NaturalCompare = 1: Exit Function
        Else
            Dim cmp As Long
            cmp = StrComp(tokA, tokB, vbTextCompare)
            If cmp <> 0 Then NaturalCompare = cmp: Exit Function
        End If
    Loop
    
    ' все токены эквивалентны — сравним длину как окончательное решение
    If Len(a) < Len(b) Then
        NaturalCompare = -1
...
' ================== Сортировка файлов в папке по natural-имени (без расширения) ==================
Function GetSortedFiles_natural(ByVal folderPath As String) As Collection
    ...
    For Each fil In fld.files
        ...
        If ext = "jpg" Or ext = "jpeg" Or ext = "png" Or ext = "tif" Or ext = "tiff" Then
            ...
            ' tmp: [0] = lowercased base name (без расширения), [1] = full path
            tmp(cnt) = Array(LCase(fso.GetBaseName(fil.Path)), fil.Path)
            cnt = cnt + 1
        End If
    Next fil
    ...
    ' сортировка вставкой с использованием NaturalCompare по tmp(...)(0)
    ...
    For i = 1 To UBound(tmp)
        ...
        j = i - 1
        Do While j >= 0
            If NaturalCompare(tmp(j)(0), keyName) = 1 Then
                tmp(j + 1) = tmp(j)
                j = j - 1
            Else
                Exit Do
...
Sub BuildAlbum()
    ...
    ' Создаём направляющие (попытка — если не поддерживается, будет fallback)
    'AddGuidesToDocument doc
